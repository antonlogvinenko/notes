...
If you replace =x= from the previous listing with an ordinary non-atomic =bool=, the behavior is guaranteed to be the same.
 ...
 * Non-atomic write to =x= happens-before the release write to =y= because of PO
 * Atomic read from =y= happens-before non-atomic read from =x= because of PO
...
#+BEGIN_SRC cpp
  ...
  #include <thread>
  #include <assert.h>
  bool x = false;                                           // (1) x is now a plain non-atomic variable
  std::atomic<bool> y;
  std::atomic<int> z;
//cake
  void write_x_then_y()
  ...
  void read_y_then_x()
  {
    ...
    if (x)                                                  // (4) This will read the value written by (1)
      ++z;
  }
  int main()
  {
