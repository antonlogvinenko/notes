...
Use =threadStatus= only for debugging purposes:
- it breaks abstractions
- its result is out of date as soon as it returns

**** Detecting Deadlocks

GHC runtime system can detect when a thread has become deadlocked and send it the =BlockedIndefinitelyOnMVar= exception.
- =MVar= and threads are objects in heap
- =MVar= points to all threads that are blocked on it
- =root set= is all runnable threads and their stacks
- if a thread is unreachable from =root set= then it is considered to be deadlocked

For example: if a *thread is blocked on an =MVar= that is not referenced by any other thread*,
then both the =MVar= and blocked thread will be unreachable, and the thread is sent =BlockedIndefinitelyOnMVar=.
Same for *being blocked on =STM=* with =BlockedIndefinitelyOnSTM= exception.

Same for a *group of mutually deadlocked threads*.
In the following example, both threads are blocked on =MVars= unreachable from the rest of the program,
so both thread will receive =BlockedIndefinitelyOnMVar=.

There are *two consequences* of this behavior.

*First*, sometimes GHC runtime will do a little more thread killing than required:
#+BEGIN_SRC
main = do
   lock <- newEmptyMVar
   ...
   takeMVar complete
#+END_SRC
Here, the runtime system decides that both main and child threads are deadlocks and kills both.
But it was only enough to kill the child thread, and that would unblock the main thread.

*Second*, sometimes GHC can't prove there is a deadlock, so it does nothing:
#+BEGIN_SRC
main = do
   lock <- newEmptyMVar
   ...
   print (lock == lock)
#+END_SRC
Here, from the point of view of runtime system, the =lock= is used not only in the child thread, so it's not considered deadlocked.

*Do not rely on deadlock detection for the correct working of your program*.
Whether =lock= is considered to be used or not (outside of some thread) depends on whether the compiler performs optimization of =isPrime 43=:
#+BEGIN_SRC
...
In Haskell, threads are lightweight and managed in user space by the runtime system.

When a Haskell *thread makes a foreign call*:
- another OS thread is created
- the responsibility of running the remaining Haskell threads is handed over to the new OS thread
- the current thread makes the foreign call

**** Foreight Out-Calls and Bound Threads

In some cases a *foreign code must be called by a particular OS thread*:
- Libraries that allow only one particluar OS thread to use their APIs
- APIs that use internal thread-local state

To solve this problem you can use *bound threads* using =forkOS=.
A *bound thread* is a Haskell thread/OS thread pair that guarantees that
foreign calls made by the Haskell thread always take place in the associated OS thread.
#+BEGIN_SRC
forkOS :: IO () -> IO ThreadId
#+END_SRC

Performance considerations:
- new OS thread is created which is more expensive than a Haskell thread
- bound Haskell threads are more expensive than unbound threads: switching between them means context-switching OS threads

=main= thread is also a bound thread, which means that its communication with other Haskell thread will lead to context-switching OS threads.

**** Asynchronous Exceptions and Foreign Calls

There is no way to interrupt a foreign call, so the runtime system waits until the call returhs before raising the exception.
The thread blocked in a foreign call is unresponsive to timeouts, interrupts, and asynchronous exceptions.

The trick is to perform the foreign call in a separate thread:
...
Do no use =withAsync=: it will block itself trying to kill the thread.

**** Threads and Foreign In-Calls

Each call of exposed Haskell API becomes a new *bound thread*:
- a new thread is created for each call
- the Haskell thread is boudn to the OS thread that made the call
 - important for dealing with GUI callbacks
